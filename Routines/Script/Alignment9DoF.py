...
def run_alignment_cpp(filename_json, filename_out):
    try:
        program = [
            "../AlignmentHeatmap/build/main", "--json", filename_json, "--out",
            filename_out
        ]
...
if __name__ == '__main__':
    ...
    outdir = "./tmp/alignments/" + os.path.basename(
        os.path.normpath(opt.projectdir)) + "/"
    ...
    for k, v in collect_results.items():
        ...
        filename_vox_scan = os.path.abspath(params["scannet_voxelized"] + "/" +
                                            id_scan + "/" + id_scan + ".vox")
        ...
        data = {
            "filename_vox_scan": filename_vox_scan,
            "catid_cad": catid_cad,
            "id_cad": id_cad,
            "rot": [
                1, -1, 0, 0
            ],  # <-- as z-up (from y-up). this rotation is needed because the up vector between shapenet and scannet is different
            "pairs": v
        }
    ...
    scenes = collections.defaultdict(
        lambda: [("# id-scan", "catid-cad", "id-cad", "tx", "ty", "tz", "qw",
                  "qx", "qy", "qz", "sx", "sy", "sz")])
    for k in collect_results.keys():
        ...
        if not os.path.exists(filename_out):
            continue
        alignments = sorted(
            CSVHelper.read(filename_out, skip_header=True), key=lambda x: x[0]
        )  # <-- sort according to alignment cost. see paper how to do proper pruning
    ...
    for k, v in scenes.items():
        ...
        q = [1, 0, 0, 0]
        s = [1, 1, 1]
        filename_scan_mesh = os.path.abspath(params["scannet"] + "/" +
                                             id_scan + "/" + id_scan +
                                             "_vh_clean_2.ply")

        scan = {
            "filename": filename_scan_mesh,
            "type": "scannet",
            "trs": {
                "trans": t,
                "rot": q,
        ...
        for item in v:
            ...
            s = item[10:13]
            # -> cad
            filename_cad_mesh = os.path.abspath(params["shapenet"] +
                                                catid_cad + "/" + id_cad +
                                                "/models/model_normalized.obj")
            cargo["meshes"].append({
                "filename": filename_cad_mesh,
                "type": "suncg_cads",
                "id_cad": id_cad,
                "catid_cad": catid_cad,
                "trs": {
