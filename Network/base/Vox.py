...
def load_vox_header(filename):
    ...
    s.grid2world = np.asarray(s.grid2world,
                              dtype=np.float32).reshape([4, 4], order=f_or_c)
...
def load_vox(filename):
    ...
    s.sdf = np.asarray(s.sdf, dtype=np.float32).reshape(
        [1, s.dims[2], s.dims[1], s.dims[0]])
    # <-
    ...
    if pdf_bytes:
        ...
        s.pdf = np.asarray(s.pdf, dtype=np.float32).reshape(
            [1, s.dims[2], s.dims[1], s.dims[0]])
    # <-
    ...
    if noc_bytes:
        ...
        s.noc = np.asarray(s.noc, dtype=np.float32).reshape(
            [3, s.dims[2], s.dims[1], s.dims[0]])
    # <-
    ...
    if bbox_bytes:
        ...
        s.bbox = np.asarray(s.bbox, dtype=np.float32).reshape(
            [1, s.dims[2], s.dims[1], s.dims[0]])
    # <-
    fin.close()
