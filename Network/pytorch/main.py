...
parser.add_argument('--train_list',
                    required=True,
                    help='path to file list of h5 train data')
parser.add_argument('--val_list',
                    default="",
                    help='path to file list of h5 val data')
parser.add_argument('--visual_list',
                    default="",
                    help="data to be visualized (saved out)")
parser.add_argument('--output',
                    default="./output/",
                    help='folder to output model checkpoints and predictions')
# train params
parser.add_argument('--mask_neg',
                    type=int,
                    required=True,
                    default=1,
                    help='mask negative samples?')
parser.add_argument('--with_match',
                    type=int,
                    required=True,
                    default=1,
                    help='train with negative samples?')
parser.add_argument('--with_scale',
                    type=int,
                    required=True,
                    default=1,
                    help='train with scale prediction?')
...
parser.add_argument('--max_iteration',
                    type=int,
                    default=int(1e6),
                    help='number of iteration to train for')
parser.add_argument('--n_samples_eval',
                    type=int,
                    default=int(1e6),
                    help='num-max to evaluate of val set')
...
parser.add_argument('--just_visualize',
                    type=bool,
                    default=False,
                    help='no training and evluation, just feed forward')

opt = parser.parse_args()

# data files
assert os.path.exists(
    opt.train_list), "train-list does not exists. Maybe data not generated?"
...
gkern3d_dim32 = torch.FloatTensor(torch.from_numpy(kernels.gaussian3d(
    7, 1.5))).view(1, 1, 7, 7, 7).cuda()
...
class TrainingWorld:
    ...
    def write_metadata(self):
        metadata = {
            "num-samples": len(self.dataset_train),
            "batch-size": opt.batch_size,
            "lr": opt.lr,
            "weight": opt.weight,
            "n-model-params": n_model_params
        }
    ...
    def start(self):
        ...
        if opt.interval_eval < 0:
            opt.interval_eval = abs(
                opt.interval_eval) * self.iterations_per_epoch
        ...
        while self.counter_iteration < (self.counter_offset +
                                        opt.max_iteration):
            #scheduler.step()
            self.train()
    ...
    def train(self):
        ...
        for i_batch, samples_batch in enumerate(self.dataloader_train):
            ...
            print("\rtraining i-iteration: %i / %d         " %
                  (self.counter_iteration,
                   opt.max_iteration + self.counter_offset))
    ...
    def save_checkpoint(self):
        torch.save(
            model.state_dict(),
            os.path.join(projectdir, "cp-%d.pth" % self.counter_iteration))
    ...
    def feed_forward(self, samples):
        ...
        mask = match_gt.clone() if opt.mask_neg else torch.ones(
            match_gt.shape).cuda()

        heatmap_gt = torch.nn.functional.conv3d(heatmap_gt,
                                                gkern3d_dim32,
                                                padding=3)
        ...
        for i in range(opt.batch_size):
            if mask[i]:
                ...
                heatmap2[i] = torch.nn.functional.log_softmax(
                    heatmap0[i].view(-1), dim=0).view(1, 32, 32, 32)
                # -> for evalulation
                heatmap3[i] = torch.nn.functional.softmax(heatmap0[i].view(-1),
                                                          dim=0).view(
                                                              1, 32, 32, 32)
                continue
                # FIXME: out of memory!
                heatmap3[i] = torch.mul(heatmap1[i] / torch.max(heatmap1[i]),
                                        heatmap3[i] / torch.max(heatmap3[i]))
                # <-

        loss = {}
        output = {}

        loss0 = losses.weighted_bce_heatmap(heatmap1,
                                            heatmap_gt,
                                            weight=opt.weight,
                                            batch_mask=mask)
        loss1 = losses.weighted_cross_entropy_heatmap(heatmap2,
                                                      heatmap_gt,
                                                      weight=opt.weight,
                                                      batch_mask=mask)
        ...
        if opt.with_match:
            ...
            loss["match"] = (losses.weighted_bce(match0.view(-1),
                                                 match_gt.view(-1)), 0.1)
        ...
        if opt.with_scale:
            loss["scale"] = (losses.mse(scale1, scale_gt,
                                        batch_mask=mask), 0.2)
        else:
            scale1 *= 0.0
    ...
    def evaluate(self):
        ...
        def run(source_list, n_max_samples=-1):
            ...
            self.dataset = dataloader.DatasetOnlineLoad(
                source_list, n_max_samples=n_max_samples)
            self.dataloader = torchdata.DataLoader(self.dataset,
                                                   batch_size=opt.batch_size,
                                                   shuffle=False,
                                                   num_workers=opt.n_threads,
                                                   drop_last=True)
            ...
            for i_batch, samples_batch in enumerate(self.dataloader):
                print("\reval: i_batch: %i / %d             " %
                      (i_batch, self.dataset.n_samples // opt.batch_size))
        ...
        print("\n##########################")
        losses_avg_train, outputs_train, pr_train = run(
            train_list, min(opt.n_samples_eval, len(train_list)))
        print("\ntrain-loss:", losses_avg_train)
        print("train-eval:", outputs_train)

        losses_avg_val, outputs_val, pr_val = run(
            val_list, min(opt.n_samples_eval, len(val_list)))
        ...
        data = {
            "iteration": self.counter_iteration,
            "loss": losses_avg,
            "eval": outputs
        }
    ...
    def visualize(self):
        ...
        self.dataloader_visual = torchdata.DataLoader(
            self.dataset_visual,
            batch_size=opt.batch_size,
            shuffle=False,
            num_workers=opt.n_threads,
        ...
        for i_batch, samples_batch in enumerate(self.dataloader_visual):
            print("\rvisualize: i_batch: %i / %d " %
                  (i_batch, self.dataset_visual.n_samples // opt.batch_size))
            losses_batch, outputs = self.feed_forward(samples_batch)
            SaveOutput.save_output(opt.batch_size, projectdir, samples_batch,
                                   outputs)
        # ---------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    projectdir, logfile, counter_iteration = user_query.setup(
        model, opt.output, opt.name)
